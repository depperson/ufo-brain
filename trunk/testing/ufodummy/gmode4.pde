...
void gmode4() {
  ...
  if (gmode4_ticks > gmode4_divider) {
    ...
    H = (float(gmode4_osc_val) / 1023);
    S = gmode4_sat;
    L = (float(gmode4_value) / 255);
    HSL(float(rotary.position())/360,S,L,Rval1,Gval1,Bval1);
    L = (float(gmode4_value2) / 255);
    HSL(float(rotary.position())/360,S,L,Rval2,Gval2,Bval2);

    g1update(Gval1);
  
    // g2 either mirrors or follows g1
    if (extmarkswitch == 1) {
       g2update(Gval2); 
    } else {
       g2update(Gval1);  
    ...
    if (phaseswitch == 0) {
      ...
      if ((horphase >= 0) && (horphase <= 1023)) {
        ...
        if (random(1024) < horphase) {
          ...
          if(random(1024) > 100) g1update(0);
          if(random(1024) > 100) g2update(0);
